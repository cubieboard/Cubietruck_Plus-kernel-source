...
static s32 get_usb_cfg(struct sunxi_hci_hcd *sunxi_hci)
{
	...
	if(type == SCIRPT_ITEM_VALUE_TYPE_PIO){
        ...
        type = script_get_item(usbc_name[sunxi_hci->usbc_no], "usb_hsic_PE16", &sunxi_hci->drv_hsic_PE16);
        if(type == SCIRPT_ITEM_VALUE_TYPE_PIO){
                sunxi_hci->drv_hsic_PE16_valid = 1;
        }else{
                DMSG_INFO("%s(drv vbus) is invalid\n", sunxi_hci->hci_name);
                sunxi_hci->drv_hsic_PE16_valid = 0;
        }

        type = script_get_item(usbc_name[sunxi_hci->usbc_no], "usb_hsic_PE17", &sunxi_hci->drv_hsic_PE17);
        if(type == SCIRPT_ITEM_VALUE_TYPE_PIO){
                sunxi_hci->drv_hsic_PE17_valid = 1;
        }else{
                DMSG_INFO("%s(drv vbus) is invalid\n", sunxi_hci->hci_name);
                sunxi_hci->drv_hsic_PE17_valid = 0;
        }

        type = script_get_item(usbc_name[sunxi_hci->usbc_no], "usb_hsic_PH5", &sunxi_hci->drv_hsic_PH5);
        if(type == SCIRPT_ITEM_VALUE_TYPE_PIO){
                sunxi_hci->drv_hsic_PH5_valid = 1;
        }else{
                DMSG_INFO("%s(drv vbus) is invalid\n", sunxi_hci->hci_name);
                sunxi_hci->drv_hsic_PH5_valid = 0;
        }
			...
			if(sunxi_hci->hsic_ctrl_flag){
				if(sunxi_hci->hsic_enable_flag){
					if(clk_prepare_enable(sunxi_hci->hsic_usbphy)){
						printk("ERR:try to prepare_enable %s_hsic_usbphy failed!\n", sunxi_hci->hci_name);
					}
				}
			}else{
				if(clk_prepare_enable(sunxi_hci->hsic_usbphy)){
...
static int alloc_pin(struct sunxi_hci_hcd *sunxi_hci)
{
	...
	if(sunxi_hci->drv_vbus_gpio_valid){
        ...
        if(sunxi_hci->drv_hsic_PE16_valid){
                ret = gpio_request(sunxi_hci->drv_hsic_PE16.gpio.gpio, NULL);
                if(ret != 0){
                        DMSG_PANIC("ERR: gpio_request failed\n");
                        sunxi_hci->drv_hsic_PE16_valid = 0;
                }else{
                        gpio_direction_output(sunxi_hci->drv_hsic_PE16.gpio.gpio, 0);
                }
        }

        if(sunxi_hci->drv_hsic_PE17_valid){
                ret = gpio_request(sunxi_hci->drv_hsic_PE17.gpio.gpio, NULL);
                if(ret != 0){
                        DMSG_PANIC("ERR: gpio_request failed\n");
                        sunxi_hci->drv_hsic_PE17_valid = 0;
                }else{
                        gpio_direction_output(sunxi_hci->drv_hsic_PE17.gpio.gpio, 0);
                }
        }

        if(sunxi_hci->drv_hsic_PH5_valid){
                ret = gpio_request(sunxi_hci->drv_hsic_PH5.gpio.gpio, NULL);
                if(ret != 0){
                        DMSG_PANIC("ERR: gpio_request failed\n");
                        sunxi_hci->drv_hsic_PH5_valid = 0;
                }else{
                        gpio_direction_output(sunxi_hci->drv_hsic_PH5.gpio.gpio, 0);
                }
        }
	...
	if(sunxi_hci->usb_host_hsic_rdy_valid){
        ...
        if(sunxi_hci->drv_hsic_PE17_valid){
                __gpio_set_value(sunxi_hci->drv_hsic_PE17.gpio.gpio, 1);
        }

        if(sunxi_hci->drv_hsic_PH5_valid){
                __gpio_set_value(sunxi_hci->drv_hsic_PH5.gpio.gpio, 1);
        }

        mdelay(10);

        if(sunxi_hci->drv_hsic_PE16_valid){
                __gpio_set_value(sunxi_hci->drv_hsic_PE16.gpio.gpio, 1);
        }
