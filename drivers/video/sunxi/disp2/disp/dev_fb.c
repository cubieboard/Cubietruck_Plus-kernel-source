...
static int Fb_map_boot_logo(__u32 sel, struct fb_info *info)
{
	...
	image_offset = vaddr;
	screen_offset = (void *)(info->screen_base);
	if(fb_width > image_width) {
		screen_offset = (void *)((u32)info->screen_base + (fb_width * ((fb_height - image_height) / 2)
	            + (fb_width - image_width) / 2) * (info->var.bits_per_pixel >> 3));
	} else if(fb_width < image_width) {
		image_offset = (void *)((u32)vaddr + (image_width * ((image_height - fb_height) / 2)
	            + (image_width - fb_width) / 2) * (info->var.bits_per_pixel >> 3));
	}

	for(i=0; i<effective_height; i++) {
		memcpy((void*)screen_offset, image_offset, effective_width*(info->var.bits_per_pixel >> 3));
		image_offset = (void*)((u32)image_offset + image_pitch*(info->var.bits_per_pixel >> 3));
		screen_offset = (void*)((u32)screen_offset + fb_width*(info->var.bits_per_pixel >> 3));
	}
